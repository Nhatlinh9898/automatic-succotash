...
const AudioVisualizer = () => {
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 text-white font-sans">
      <div className="p-4 lg:p-5 bg-black/30 backdrop-blur-md border-b border-white/10 flex flex-col lg:flex-row justify-between items-center gap-4">
        <h2 className="m-0 text-2xl lg:text-3xl font-bold text-white drop-shadow-lg">🎵 Audio Visualizer</h2>
        <div className="flex items-center">
          <span className={`px-4 py-2 rounded-full border text-sm font-medium transition-all duration-300 ${
            isPlaying 
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
        <div className="w-full lg:w-96 bg-black/30 backdrop-blur-md p-4 lg:p-5 overflow-y-auto border-b lg:border-b-0 lg:border-r border-white/10">
          <div className="mb-8">
            <h3 className="m-0 mb-4 text-xl lg:text-xl font-semibold text-white border-b border-white/20 pb-2">Audio Source</h3>
            <div className="flex flex-col gap-3 mb-4">
              ...
              <label 
                htmlFor="audio-file" 
                className="px-4 py-3 bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg text-white cursor-pointer text-sm font-medium text-center transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-blue-500/40"
              >
                📁 Choose Audio File
              </label>
              {audioFile && (
                <span className="text-xs text-center py-2 bg-white/10 rounded-lg opacity-80">
                  {audioFile.name}
                </span>
              )}
            </div>
            ...
            <div className="mb-4">
              <button
                ...
                className={`w-full px-4 py-3 rounded-lg text-white cursor-pointer text-sm font-medium transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg ${
                  isPlaying 
            ...
            {audioElementRef.current && (
              <div className="flex items-center gap-4 p-3 bg-white/10 rounded-lg">
                <button
                  ...
                  className="px-3 py-2 bg-gradient-to-r from-orange-500 to-orange-600 rounded-lg text-white cursor-pointer text-base transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-orange-500/40"
                >
                  {isPlaying ? '⏸️' : '▶️'}
                </button>
          ...
          <div className="mb-8">
            <h3 className="m-0 mb-4 text-xl lg:text-xl font-semibold text-white border-b border-white/20 pb-2">Visualization Type</h3>
            <div className="grid grid-cols-2 lg:grid-cols-2 gap-3">
              {visualizationTypes.map(type => (
                <button
                  key={type.id}
                  className={`flex flex-col items-center p-4 bg-white/10 border border-white/20 rounded-lg text-white cursor-pointer transition-all duration-300 text-center hover:bg-white/20 hover:-translate-y-0.5 ${
                    visualizationType === type.id ? 'bg-white/30 border-white/50 shadow-lg shadow-white/20' : ''
                  }`}
                  onClick={() => setVisualizationType(type.id)}
                >
                  <span className="text-2xl lg:text-3xl mb-2">{type.icon}</span>
                  <span className="font-bold text-sm mb-1">{type.name}</span>
                  <span className="text-xs opacity-80 leading-tight">{type.description}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="mb-8">
            <h3 className="m-0 mb-4 text-xl lg:text-xl font-semibold text-white border-b border-white/20 pb-2">Visualizer Settings</h3>
            
            <div className="mb-5">
              <label className="block mb-2 text-sm font-medium opacity-90">Sensitivity: {sensitivity.toFixed(1)}</label>
              <input
                type="range"
                min="0.5"
                max="3"
                ...
                className="w-full h-1.5 bg-white/20 rounded-lg outline-none appearance-none cursor-pointer slider"
              />
            </div>

            <div className="mb-5">
              <label className="block mb-2 text-sm font-medium opacity-90">Smoothing: {smoothing.toFixed(2)}</label>
              <input
                type="range"
                min="0"
                max="0.99"
            ...
            <div className="mb-5">
              <label className="block mb-2 text-sm font-medium opacity-90">Color Scheme</label>
              <select
                value={colorScheme}
                onChange={(e) => setColorScheme(e.target.value)}
                className="w-full p-2.5 bg-white/10 border border-white/20 rounded-lg text-white text-sm cursor-pointer"
              >
                {colorSchemes.map(scheme => (
                  <option key={scheme.id} value={scheme.id} className="bg-gray-800 text-white">
                    {scheme.name}
                  </option>
                ))}
              </select>
            ...
            {visualizationType === 'particles' && (
              <div className="mb-5">
                <label className="block mb-2 text-sm font-medium opacity-90">Particle Count: {particleCount}</label>
                <input
                  type="range"
                  min="100"
                  max="2000"
          ...
          <div className="bg-white/5 rounded-xl p-4 border border-white/10">
            <h3 className="m-0 mb-4 text-xl lg:text-xl font-semibold text-white border-b border-white/20 pb-2">Frequency Spectrum</h3>
            <div className="flex items-end h-16 gap-0.5">
              {frequencyData.slice(0, 32).map((value, index) => (
                <div
                  key={index}
                  className="flex-1 bg-gradient-to-t from-blue-500 to-purple-500 rounded-t-sm min-h-[2px] transition-all duration-100"
                  style={{
        ...
        <div className="flex-1 relative flex flex-col">
          <div ref={mountRef} className="flex-1 bg-gradient-to-br from-gray-900 via-gray-800 to-black rounded-xl m-3 lg:m-5 overflow-hidden shadow-2xl" />
          <audio
            ref={audioElementRef}
          ...
          <div className="absolute bottom-5 lg:bottom-8 left-5 lg:left-8 bg-black/70 backdrop-blur-md p-3 lg:p-5 rounded-xl border border-white/10 max-w-xs lg:max-w-sm">
            <h4 className="m-0 mb-2 lg:mb-2.5 text-base lg:text-lg lg:text-xl font-semibold text-white">3D Audio Visualization</h4>
            <p className="m-0 text-xs lg:text-sm opacity-80 text-gray-300">Type: {visualizationTypes.find(t => t.id === visualizationType)?.name}</p>
            <p className="m-0 text-xs lg:text-sm opacity-80 text-gray-300">Color Scheme: {getColorScheme().name}</p>
            <p className="m-0 text-xs lg:text-sm opacity-80 text-gray-300">Status: {isPlaying ? 'Playing' : 'Stopped'}</p>
            <p className="text-blue-400 text-xs lg:text-sm italic mt-2 lg:mt-2.5">
